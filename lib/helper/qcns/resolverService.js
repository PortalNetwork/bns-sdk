...
var resolverInit = exports.resolverInit = function resolverInit(provider, resolverAddr) {
  ...
  if (!resolver) {
    resolver = new _resolver2.default(web3, _quarkchainWeb2.default.getQkcAddressFromEthAddress(resolverAddr));
  }
};
...
var getMultihash = exports.getMultihash = async function getMultihash(name, key) {
  try {
    var content = await resolver.multihash(namehash.hash(name), key);
    return content;
  } catch (err) {
