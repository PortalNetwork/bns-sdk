...
var owner = exports.owner = async function owner(name) {
  try {
    ...
    var res = await neon.rpc.Query.invokeScript(ownerScript).execute(provider); // parameter
...
var getIPFS = exports.getIPFS = async function getIPFS(name) {
  try {
    ...
    var res = await neon.rpc.Query.invokeScript(IPFSScript).execute(provider); // parameter
